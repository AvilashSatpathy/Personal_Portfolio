...
$(document).ready(function() {
  ...
  const images = $('.img-fluid');
  let currentIndex = 0;
  let isScrolling = false; // Flag to prevent multiple animations

  function scrollToNextImage() {
    if (!isScrolling) {
      isScrolling = true;
      container.animate({
        scrollLeft: container.scrollLeft() + images.eq(currentIndex).width()
...
$(document).ready(function () {
  const container = $(".scrolling-container");
  const images = $(".img-fluid");
  const imageWidth = images.first().outerWidth(true); // Get the width of an image including margins
  let currentIndex = 0;
  ...
  function scrollToIndex(index) {
    if (!isScrolling) {
      isScrolling = true;
      const scrollPosition = index * imageWidth; // Calculate the scroll position based on the index
      container.animate(
        {
          scrollLeft: scrollPosition, // Move directly to the target position
        },
        1000,
        "swing",
        function () {
  ...
  // Event listeners for manual control (arrow keys)
